...
describe("Configuration", function () {
    describe("fromPath", function () {
        ...
        it("reads the configuration from 'lerna.json'", function () {
            ...
            const result = (0, configuration_1.fromPath)(tmpDir);
        ...
        it("throws ConfigurationError if neither 'package.json' nor 'lerna.json' exist", function () {
            expect(() => (0, configuration_1.fromPath)(tmpDir)).toThrowError(configuration_error_1.default);
        });
        it("uses passed in `repo` option", function () {
            const result = (0, configuration_1.fromPath)(tmpDir, { repo: "foo/bar" });
    ...
    describe("findRepoFromPkg", function () {
        ...
        tests.forEach(([input, output]) => {
            it(`'${input}' -> '${output}'`, function () {
                expect((0, configuration_1.findRepoFromPkg)({
                    repository: {
                        type: "git",
                        url: input,
                    },
